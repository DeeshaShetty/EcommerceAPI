...
public class EcommerceAPITest {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		RequestSpecification reqLogin=given().relaxedHTTPSValidation().log().all()
				.spec(reqspec).body(loginRequest);
	...
	//	Add Product 
		RequestSpecification addProductReqSpec = new RequestSpecBuilder()
				.setBaseUri("https://rahulshettyacademy.com").addFilter(RequestLoggingFilter.logRequestTo(log))
				.addFilter(ResponseLoggingFilter.logResponseTo(log))
		...
		RequestSpecification createOrderReq=given().log().all()
				.spec(createorderReqSpec).body(orders);
		String responseAddOrder=createOrderReq.when()
				.post("/api/ecom/order/create-order").then().log().all()
...
//		Delete Product
		RequestSpecification deleteProdReq=given().log().all()
				.spec(createOrderReq).pathParam("productId",productId);
		...
		//Delete Order
		RequestSpecification deleteOrdReq=given().log().all()
				.spec(createOrderReq).pathParam("OrderId",OId);
		String deleteOrderResponse=deleteOrdReq.when()
